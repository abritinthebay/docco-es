...
const copyLayoutFactory = function(config, callback) {
	return function(error) {
		...
		if (config.public) {
			if (typeof config.public === "string" && fs.existsSync(config.public)) {
				// If the public assets are a simple string path just copy it to the right place
				return copyAsset(config.public, config.output, callback);
			}
			if (Array.isArray(config.public)) {
				// However if they are an array then it's file data (from a zip)
				config.public.forEach((file) => {
					fs.outputFileSync(path.join(config.output, file.name), file.data);
				});
			}
		}
		return callback();
...
export const document = function(options = {}, callback = errorCallback) {
	...
	if (!config) {
		return global.console.log("docco: invalid configuration, exiting.");
	}
